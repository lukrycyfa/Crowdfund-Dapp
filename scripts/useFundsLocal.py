...
def donate_mint():
    while True:
        ...
        accIdx = enterIdx(8)    
        img_lst = os.listdir("./token/src/NFTS/images")
        ...
        tokenId = curtoken + 1  
        with open(f"./token/src/NFTS/metadata/{img_lst[get_nft][0:-4]}.json", 'r') as meta_file:    
            file = json.load(meta_file)
            new_meta = {}
            new_meta['image'], new_meta['owner'], new_meta['tokenId'] = f"./token/src/NFTS/images/{img_lst[get_nft]}", str(accounts[accIdx]), tokenId
        ...
        try:        
            ...
            Funds.safeMint(f"./token/src/NFTS/metadata/{img_lst[get_nft][0:-4]}.json", {'from':accounts[accIdx], 'gas_price':gas_strategy})
            new_meta['metaUrl'] = f"./token/src/NFTS/metadata/{img_lst[get_nft][0:-4]}.json"  
            with open(f"./token/src/NFTS/awardlocal/awd_{tokenId}.json", 'w') as awd_meta:
                json.dump(new_meta, awd_meta, indent=4)
            print("Token Minted Succefully, and a copy created in ./token/src/NFTS/awardlocal/ dir"), print(" "), print(" ")
        except Exception as e:
                print(e.args)                           
