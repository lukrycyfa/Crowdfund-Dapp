...
def donate_mint():
    while True:
        ...
        img_lst = os.listdir("./crowd-fund-app/src/NFTS/images")
        donation = enterValDonation()
        for i, r in enumerate(img_lst):
            print(f"{i}.", r)
        print("Pick One Nft To Mint")
        get_nft = enterIdx(len(img_lst))
        with open(f"./crowd-fund-app/src/NFTS/images/{img_lst[get_nft]}", 'rb') as img_file: # this block of code downwards require network
            try:
                ...
                if upd[1].status_code == 200:
                    print(upd[0])            
                    with open(f"./crowd-fund-app/src/NFTS/metadata/{img_lst[get_nft][0:-4]}.json", 'r') as meta_file:    
